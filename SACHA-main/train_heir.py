...
learning_rate_pi = config['lr_pi']
learning_rate_q = config['lr_q']
...
def main(args):
    ...
    env = make_parallel_env(num_rollout_threads)
    model = AttentionPolicy(args.communication, input_shape, output_shape,hidden_channels, hidden_dim, num_heads, num_agents)
    
    # Define Adam optimizer for policy and critic networks
    policy_optimizer = torch.optim.Adam(algo.policy.parameters(), lr=learning_rate_pi, betas=betas, eps=eps, weight_decay=weight_decay)
    critic_optimizer = torch.optim.Adam(algo.critic.parameters(), lr=learning_rate_q, betas=betas, eps=eps, weight_decay=weight_decay)
    ...
    for e in range(0, num_episodes, num_rollout_threads):
        ...
        for t in range(config['max_episode_length']):
            ...
            if num_agents > threshold:
                groups,single_heuristics, paired_heuristics = AttentionPolicy.group_agents(model, b_state)
                if(single_heuristics < paired_heuristics): 
                    # Coordinate groups towards goal state
                    POMAPFEnv.coordinate_groups_to_goal_state(groups, POMAPFEnv.goal_state)
                    # Coordinate agents within each group
                    for group in POMAPFEnv.group:
                        POMAPFEnv.coordinate_agents_within_group(group, POMAPFEnv.goal_state)
            ...
            buffer.add(b_obs, b_action, b_reward, b_next_obs, b_done, 
            b_info, b_hidden, b_state)
        ...
        if (e+1) % config['save_interval'] == 0:
            algo.prep_rollout(rollout_device)            
            save_path = os.path.join(log_dir, f"heir_model_ep{e+1}.pth")
            algo.save(save_path)           
